...
def check_python(
    minimum: str = "3.8.0", hard: bool = True, verbose: bool = True
) -> bool:
    """
...
def check_amp(model):
    ...
    prefix = colorstr("AMP: ")
    LOGGER.info(
        f"{prefix}running Automatic Mixed Precision (AMP) checks with YOLO11n..."
    )
    ...
    try:
        ...
        LOGGER.warning(
            f"{prefix}checks skipped ⚠️, offline and unable to download YOLO11n. {warning_msg}"
        )
